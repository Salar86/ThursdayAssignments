...
public class Board {
    ...
    private static ArrayList<ChanceCard> c1 = new ArrayList<>();
    ...
    private static String createChanceCards(String[] chanceData) {
        String txt = "";
        for (String s : chanceData) {
            String[] values = s.split(",");
            txt = values[0];
            int cost = Integer.parseInt(values[1]);
            int income = Integer.parseInt(values[2]);
            c1.add(new ChanceCard (txt, cost, income));
        }
        return txt;
    }

    public static ChanceCard getChanceCard(){
        ChanceCard c2 =  c1.get(index);
        index++;
        return c2;
